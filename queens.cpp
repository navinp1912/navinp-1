...
int go (int row, int col, int lmask, int rmask,int lastcol,int colmask)
{
  int i, j;
...
//      if (colmask > 0)
  ...
  int sum = 0;
  if (arr[row][col] == '#')
    return dp[row][col][lmask][rmask][slastcol][colmask]= go (row, col + 1, lmask & ~(1 << col), rmask & ~(1 << col),col,colmask & ~(1 << col))%MOD;
  ...
  if ((((lmask) & (1 << col)) == 0) and (((rmask) & (1 << col)) == 0))
	...
	{
//	  printf (" choosing %d %d\n", row, col);
	  sum += go (row, col + 1, (lmask) | (1 << col), (rmask) | (1 << col),col,  colmask | (1 << col))%MOD;
	}
    }
    sum += go (row, col + 1, lmask, rmask,lastcol, colmask)%MOD;


  return dp[row][col][lmask][rmask][slastcol][colmask]=sum%MOD;
}
